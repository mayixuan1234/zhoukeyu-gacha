...
| 指标 | 数值 |
|------|------|
| 真实用户 | 1,000+（粉丝社群活动，7 天内从 0 增长） |
| 全站总抽数 | 111,801 |
| 全站总销售额 | ¥737,886 |
| 多端覆盖 | 网页版 + 微信小程序 + Android APK |
| 关键能力 | Excel 导入 / OCR 双引擎识别 / 排行榜 / 碎卡兑换 / 微信内核兼容 |

- **有真实业务场景**：为粉丝后援会盲盒抽卡活动而做，活动方与粉丝实际使用
- **从 0 到 1000+ 用户只用了 3 天**（v1 发布 → v6 在多个大型微博群传播）
- **解决真实工程问题**：微信清 localStorage 丢数据、OCR 识别准确率、排行榜防造假、APK 签名管理（详见「踩过的坑」）
...
> ⚠️ 签名证书 `focus.keystore` **不能删、不能公开**——证书丢了就得先卸载才能装，数据全没。证书与密码只保存在本地与网盘备份中。
...
- **排行榜**：全站粉丝抽数 Top50 云端榜单——昵称上榜、我的排名、防造假校验（「排行榜是增长引擎」落地实现）
...
**解决**：上报抽数做合理性校验（超过全站总抽数即拒绝），榜单标注"由用户自主上报，仅供参考"；数据存云端 KV，本地只存设备标识与昵称。
...
**解决**：`focus.keystore` 备份到多台设备 + 网盘（证书密码不写入仓库，仅保存在本地与网盘）。
...
| 阶段 | 时间 | 用户数 | 增长方式 |
|------|------|--------|----------|
| v1 | 7.13 凌晨 | 个位数 | 发布微博"做了个统计工具" |
| v2-v3 | 7.13 白天 | 几十人 | 好友转发 |
| v4-v5 | 7.14 | 几百人 | 加了 OCR，更新博文"现在截图也能识别了" |
| v6 | 7.15 | 1000+ | 持续更新功能 在多个大型微博群里传播 |
...
├── functions/api/draws.js           # 云端总抽数 API
├── functions/api/rank.js             # 排行榜 API（KV 存储）
...
| v7 | 2026.9.16 | 排行榜上线：全站 Top50 云端榜单 + 昵称上榜 + 防造假校验 |
